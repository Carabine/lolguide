...
export class HtmlService {
  ...
  async renderMainPage(lang) {
    ...
    return {
      ...
      description: {
        en: "Unlock Victory with League of Legends Guides! Discover Strategies, Champion Tips, and Winning Tactics on LoLGuide. Dominate and Climb the Ranks with our Comprehensive Resources",
        ru: "Достигните победы с гайдами по Лиге Легенд! Откройте для себя стратегии, советы по чемпионам и выигрышные тактики на LoLGuide. Доминируйте и поднимайтесь в рейтинге с нашими всесторонними ресурсами"
      },
      i18n, articles, lang
    };
  }
  ...
  async renderGuidesPage(lang) {
    ...
    return {
      ...
      metaTitle: {en: "All guides - LoLGuide", ru: "Все гайды - LoLGuide"},
      // description: {en: articles.map(a => a.champion.name["en"]).join(" · "), ru: articles.map(a => a.champion.name["ru"]).join(" · ")},
      description: {
